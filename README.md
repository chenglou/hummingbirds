...
Nobody waits on the line. Every question carries `x-hummingbirds-reply-to: <the sender's address>`; the bird answers 202 right away, runs its turn in its own time, and POSTs its reply there itself, tagged `x-hummingbirds-in-reply-to`. Codex sees each message behind a few header lines (who it's from, the request id, where to reply), which is all a bird needs to tell a late reply from a new question. The harness holds no per-request state for this; the conversation does.

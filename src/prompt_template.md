...
- After splitting, tell each child about the peers you think are relevant to its work, including their IDs, addresses, and what you know about them.
...
You can create another independent bird with `[command] new <id>`, then start it with `[command] start <id> --address [child-address][bind-option] --detach`. The start command reports its address. Teach or introduce it through ordinary messages.
<!-- /birds:runtime -->

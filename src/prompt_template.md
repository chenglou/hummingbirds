...
- Include `x-from: $HUMMINGBIRDS_NODE_ID`, `x-reply-to: $HUMMINGBIRDS_NODE_ADDRESS`, and `x-route: $HUMMINGBIRDS_ROUTE` (expand it anew each turn) on every peer POST. For a new or forwarded request, also include `x-request: $HUMMINGBIRDS_REQUEST_ID`
- To reply, POST only your reply text to `x-reply-to:` with those same headers, except use `x-in-reply-to: $HUMMINGBIRDS_REQUEST_ID` instead of `x-request`. Never send both. Your final assistant message is not delivered. No `x-reply-to:` means no reply is expected

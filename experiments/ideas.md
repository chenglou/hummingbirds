...
- Before the prompt said which part of a message to forward, a bird forwarded the whole envelope, `Reply-to:` included, so three birds each replied to the human. Model slips with `curl` (a body left in a shell variable, an unset variable) produced empty replies twice; the bird recovered once on its own. Birds and the inbox now answer 400 to an empty body so the next slip is visible to the sender. With the original prompt voice restored, a bird also copied the `From:`/`Re:` lines into its own replies in one run; harmless, worth a one-line note only if it keeps happening. If slips keep happening, the fix is probably a note in the prompt about `--data-binary @file`, not more harness.
